...
class EinwahlManager {
    ...
    updateZweitwunschOptions(erstwunschId) {
        // Reset Zweitwunsch
        this.zweitwunschSelect.innerHTML = '<option value="">Bitte wählen...</option>';
        
        if (!erstwunschId) {
            this.zweitwunschSelect.disabled = true;
            this.updateSelectInfo(this.zweitwunschSelect, 'Erst Erstwunsch wählen...');
            return;
        }
        ...
        this.schwerpunkte.forEach(sp => {
            ...
            if (kombinierbar && !istVoll) {
                ...
                option.value = sp.id;
                option.textContent = `${sp.name} (${aktuell}/${max})`;
    ...
    sindKombinierbar(schwerpunkt1, schwerpunkt2) {
        ...
        if (schwerpunkt1.kombination_mit && schwerpunkt1.kombination_mit != schwerpunkt2.id) {
            return false;
        }
        
        // Wenn Schwerpunkt2 eine Pflicht-Kombination hat
        if (schwerpunkt2.kombination_mit && schwerpunkt2.kombination_mit != schwerpunkt1.id) {
            return false;
        }
    ...
    updateSelectInfo(selectElement, message, type = 'info') {
        ...
        infoElement.className = `course-info ${type === 'error' ? 'course-full' : ''}`;
    }
    ...
    handleFormSubmit(event) {
        ...
        if (!vorname || !nachname || !klasse_id || !erstwunsch_id || !zweitwunsch_id) {
        ...
        if (!this.sindKombinierbar(erstwunsch, zweitwunsch)) {
            event.preventDefault();
            this.showValidationError('Diese Schwerpunkt-Kombination ist nicht erlaubt.');
            return;
        }
